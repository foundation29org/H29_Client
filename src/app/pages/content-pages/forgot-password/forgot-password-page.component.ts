...
export class ForgotPasswordPageComponent implements OnDestroy{
    ...
    onSubmit() {
      ...
      .subscribe( (res : any) => {
        ...
        swal('', this.translate.instant("recoverpass.Account recovery email sent"), "success");
       }, (err) => {
         this.sending = false;
         this.forgotPasswordForm.reset();
         //errores de fallos
         ...
         if(errormsg == 'Fail sending email'){
           ...
           swal(this.translate.instant("generics.Warning"),this.translate.instant("login.The account is not activated"), "error");
         }else if(errormsg == 'user not exists'){
           swal(this.translate.instant("generics.Warning"), this.translate.instant("recoverpass.no account"), "error");
         }else{
           swal(this.translate.instant("generics.Warning"), this.translate.instant("generics.error try again"), "error");
         }
       }));
