...
import { QnaPair } from 'app/admin/manage-faqs-menu/manage-faqs/qna-pair.interface';
...
export class ManageFaqsComponent implements OnDestroy{

  dataorigin:any=null;
  dataqnatemp:any=null;
  lang: string = '';
  groups: Array<any> = [];
  ...
  constructor(private http: HttpClient, public translate: TranslateService, private authService: AuthService, private authGuard: AuthGuard, private langService: LangService, private faqService: FaqService, public toastr: ToastsManager, private modalService: NgbModal){
  ...
  onChangeGroup(value){
    ...
    this.authService.setGroup(value);
  ...
  getLastPublishedTimestamp(knowledgeBaseID){
   ...
   newKnowledgesLang(){
    ...
    createKnowledgeBase(lang, knowledgeBaseID){
      ...
      this.subscription.add( this.http.post(environment.api+'/api/superadmin/qna/'+this.authService.getIdUser(),paramssend)
        .subscribe( (res : any) => {
          this.loadingFaqs = false;
          this.loadAvailablesQnas();
         }, (err) => {
    ...
    confirmDeleteKnowledge(){
      this.subscription.add( this.http.delete(URLQna+this.actualKnowledge.knowledgeBaseID)
        .subscribe( (res : any) => {
          ...
          this.subscription.add( this.http.delete(environment.api+'/api/superadmin/qna/'+paramssend)
            .subscribe( (res : any) => {
    ...
    updateQuestion(id,index){
      ...
      this.actualQna.questions= copyQuestions;
      for (var j = 0; j < this.actualQna.metadata.length; j++) {
        if(this.actualQna.metadata[j].name=='category'){
          this.selectedItems.push(this.actualQna.metadata[j].value)
        }
      }
    }
